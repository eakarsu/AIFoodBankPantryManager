...
router.get('/', async (req, res) => {
  try {
    ...
    const { rows: countRows } = await db.query('SELECT COUNT(*) AS total FROM volunteers');
    const total = parseInt(countRows[0].total, 10);
    const { rows } = await db.query('SELECT * FROM volunteers ORDER BY id DESC LIMIT $1 OFFSET $2', [limit, offset]);
